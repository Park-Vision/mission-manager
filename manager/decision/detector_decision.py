...
class DetectorDecision(Decision):
    ...
    def object_is_big(self, box_xyxy: ndarray, image_dimensions: ImageDimensions) -> bool:
        """Returns true if detected object bounding box takes up defined proportion of screen"""
        return abs(box_xyxy[0] - box_xyxy[2]) > image_dimensions.width * 0.1 and abs(box_xyxy[1] - box_xyxy[3]) > image_dimensions.height * 0.1
    
    def object_is_central(self, box_xyxy, image_dimensions: ImageDimensions) -> bool:
        """Returns true if detected object bounding box center is within a defined distance to image center"""
        return abs(((box_xyxy[0] + box_xyxy[2]) / 2) - image_dimensions.center_x) < image_dimensions.width * 0.25 and abs(((box_xyxy[1] + box_xyxy[3]) / 2) - image_dimensions.center_y) < image_dimensions.height * 0.25
    ...
    async def decide(self, wp: Waypoint) -> bool:
        ...
        image_dimensions = ImageDimensions(image_width, image_height, image_width / 2, image_height / 2)
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Decide if detected object takes parking space. It has to be big enough and positioned in the center of image
                if self.object_is_big(r, image_dimensions) and self.object_is_central(r, image_dimensions):
                    spot_free = False

        # Draw picture with boxes for testing
        # self.visualize_result(result)

        self.free_spots.append({"parking_spot_id": wp.spot_id, "occupied": not spot_free})
